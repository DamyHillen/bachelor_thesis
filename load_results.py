...
def main():
    ...
    t = time.time()
    # res1 = StateResults("results/states/[1-10]_150y_1000a_mu.states")

    # res1 = ErrorResults("results/errors/[10]_150y_1000a_mu.errors")
...
def plot_errors(results):
    ...
    if len(results) > 1:
        ...
        result = results[0]
        errors = np.array(result.model_errors).T
        plt.plot(np.arange(result.N_ITER)/result.YEAR_LEN, errors, COLORS[0], alpha=0.01, zorder=-1)
        plt.ylim((0, max))
        end_max = np.max(errors[-result.YEAR_LEN:, :])
        plt.text(x=100, y=end_max + 8, s="ε = {:.2f}".format(end_max))
        plt.hlines(y=end_max, xmin=0, xmax=result.N_ITER / result.YEAR_LEN, colors=['black'], zorder=1, linewidths=[2], linestyle='dashed', label="Max resulting error")
        plt.xlabel("Time (years)")
        plt.ylabel("Model error (ε)")
        plt.legend(loc="upper right")
        plt.title("Model errors of {} agents with random priors\nLayer states: {}".format(len(results[0].model_errors), result.LAYER_STATES))
...
def plot_state_results(results):
    ...
    for layer in range(layer_count):
        ...
        axs[2, layer].set_xlabel("Time (years)")
        axs[2, layer].set_ylabel("Equilibrium")
